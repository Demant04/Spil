...
func _update_sell_display():
	var iron_amount = GameState.get_resource_amount("iron")
...
func _update_upgrade_display():
	if GameState.has_upgrade("cargo_upgrade_1"):
		...
		if GameState.credits >= CARGO_UPGRADE_COST:
...
func _on_sell_pressed():
	...
	# Remove all iron
	GameState.remove_resource("iron", iron_amount)
...
func _on_upgrade_pressed():
	if GameState.purchase_upgrade("cargo_upgrade_1", CARGO_UPGRADE_COST):
		_update_display()
